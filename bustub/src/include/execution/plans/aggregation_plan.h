...
class AggregationPlanNode : public AbstractPlanNode {
 public:
  ...
  std::vector<AbstractExpressionRef> group_bys_; // group by后面的参数（按照哪些列的值分组）
  /** The aggregation expressions */
  std::vector<AbstractExpressionRef> aggregates_; // min/max/count/count(*)对应的那些列
  /** The aggregation types */  
  std::vector<AggregationType> agg_types_;        // 聚集函数的类型
...
struct AggregateKey {
  /** The group-by values */
  std::vector<Value> group_bys_;    // 85行是分组的列，这里是按照列具体分成了哪些值，这里对应的是一个分组
...
struct AggregateValue {
  ...
  std::vector<Value> aggregates_;   // 这里对应的是87、89两个按照列具体分组之后聚合函数计算出来的值，这里对应的是某一个分组
};
