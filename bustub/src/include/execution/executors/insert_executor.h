...
class InsertExecutor : public AbstractExecutor {
 ...
 private:
  ...
  const TableInfo *table_info_;       // 表的信息
  std::vector<IndexInfo*> table_indexes_info_; // 表关联的索引信息
  std::unique_ptr<AbstractExecutor> child_executor_;  // 子执行器的指针
  bool has_no_tuple_;   // 是否有元组需要被插入，true：没有，false：有，初始化的时候默认是false
};
