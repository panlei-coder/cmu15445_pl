...
class LockManager {
 public:
  ...
  ~LockManager() {
    ...
    cycle_detection_thread_->join(); // 等待cycle_detection_thread_线程结束，并阻塞当前线程，避免资源泄漏
  ...
  // my add
  // 判断两个锁是否满足升级的条件
  auto IsCompatible(LockMode lock_mode_before,LockMode lock_mode_after) -> bool;
  // 锁的授予是否是可以兼容的
  auto GrantCompatiable(LockMode lock_mode_first,LockMode lock_mode_second) -> bool;
  // 基于不同的隔离级别下，事务释放不同的锁，事务的状态也相应的需要发生改变
  void UnLockChangeState(Transaction *txn,LockMode lock_mode);

  /************************************************表锁*********************************************/
  // 输出加锁信息的日志
  void Log(Transaction *txn,LockMode lock_mode,table_oid_t oid);
  // 获取表锁的等待队列
  auto GetLRQueuePtr(table_oid_t oid) -> std::shared_ptr<LockRequestQueue>;
  // 将某个锁添加到事务txn的锁集合中
  void BookKeeping(Transaction *txn,LockMode lock_mode,table_oid_t oid);
  // 从事务txn的锁集合中删除某个锁
  void BookKeepingRemove(Transaction *txn,LockMode lock_mode,table_oid_t oid);
  // 更新表锁，将新申请的锁添加到等待队列中
  ...
  /************************************************行锁*********************************************/
  // 输出加锁信息的日志
  void Log(Transaction *txn,LockMode lock_mode,RID rid);
  // 获取行锁的等待队列
  auto GetLRQueuePtr(RID rid) -> std::shared_ptr<LockRequestQueue>;
  // 加行锁之前，需要检查表锁和行锁之间的逻辑是否相符
  void IsTableFit(Transaction *txn,LockMode lock_mode,table_oid_t oid);
  // 将某个锁添加到事务txn的锁集合中
  void BookKeeping(Transaction *txn,LockMode lock_mode,table_oid_t oid,RID rid);
  // 从事务txn的锁集合中删除某个锁
  void BookKeepingRemove(Transaction *txn,LockMode lock_mode,table_oid_t oid,RID rid);
  // 更新行锁，将新申请的锁添加到等待队列中
  auto UpdateLock(Transaction *txn,LockMode lock_mode,table_oid_t oid,RID rid) -> bool;
  // 行锁的授予
  auto GrantLock(Transaction *txn,LockMode lock_mode,RID rid) -> bool;

  // 死锁检测的DFS算法，检测等待图中是否存在环
  auto DFS(txn_id_t txn_id,std::stack<txn_id_t> &cycle_stack,
 ...
 private:
  ...
  // 存放死锁检测等待图中所有的事务id，并标记事务是否从等待图中剔除了（1：在等待图中，0：已经被剔除了）
  std::unordered_map<txn_id_t, int> txn_id_set_; 
